...
const EditSermon = () => {
    ...
    useEffect(() => {
        ...
        const sermonDateToISO = (dateStr: string) => {
            if (!dateStr) return "";
    ...
    const handleShare = (platform: 'facebook' | 'whatsapp') => {
        if (!formData.slug) {
            toast.error("Le sermon doit être enregistré pour être partagé");
            return;
        }
        const url = `${window.location.origin}/sermon/${formData.slug}`;
                            ...
                            {(formData.content_type === "youtube" || formData.content_type === "video") ? (
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <label htmlFor="sermon-media-source" className="text-[10px] text-gray-400 uppercase font-bold tracking-tight cursor-pointer block">{t("admin.sermons_page.form.video_url_help")}</label>
                                        <input
